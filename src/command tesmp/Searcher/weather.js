...
module.exports = class Weather extends Command {
	...
	async run(bot, message, args, settings) {
		...
		find({ search: args.join(' '), degreeType: 'F' }, function(err, result) {
			try {
				const embed = new MessageEmbed()
					...
					.addField(message.translate(settings.Language, 'SEARCHER/WEATHER_TEMP'), `${result[0].current.temperature}°F`, true)
